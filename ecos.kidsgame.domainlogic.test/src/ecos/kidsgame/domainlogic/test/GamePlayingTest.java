...
public class GamePlayingTest {
	...
	Collection<Palabra> coleccion = new ArrayList<Palabra>(Arrays.asList(new Palabra[] { casa, cabra, cebra, salamanca, cereal }));
	return coleccion;
    }
    ...
    private boolean comprobarSiLaSilabaCoincideConAlgunaDeLasSilabasDeLaColeccionPasada(Silaba silabaDevueltaEnLaPrueba, Collection<Silaba> silabasUsadasParaIniciarLaPrueba) {
	...
	return String.format("Hola {0} este es el juego de las sílabas. Tienes que pasar por varias pruebas para terminarlo", usuario.getNombre());
    }
	...
	JuegoDeSilabas juego = new JuegoDeSilabas(usuario, silabas);
	SeleccionarLasSilabasIndicadas seleccionarLasSilabas = juego.getSeleccionarLasSilabasIndicadas();
	...
	JuegoDeSilabas juego = new JuegoDeSilabas(usuario, silabas);
	CompletarPalabrasConSilabas completarPalabrasConSilabas = juego.getCompletarPalabrasConSilabas(palabras);
	...
	CompletarPalabrasConSilabas completarPalabrasConSilabas = juego.getCompletarPalabrasConSilabas(palabrasParametro);
	...
	for (int i = 0; i < palabras.size(); i++) {
	    ...
	    assertTrue(comprobarSiLaSilabaCoincideConAlgunaDeLasSilabasDeLaColeccionPasada(informacionPendiente.getSilaba(), silabas));

	    Silaba silabaDeLaColeccionQueCorresponde = consultarQueSilabaDeLaColeccionCoincideConLaSilaba(silabas, informacionPendiente.getSilaba());
	    Silaba silabaDeLaColeccionQueNOCorresponde = obtenerUnaSilabaDeLaColeccionQueNoCoincideConLaSilaba(silabas, informacionPendiente.getSilaba());
    ...
    private boolean comprobarSiLaPalabraCompletaCoincideConAlgunaDeLasPalabrasDeLaColeccion(Palabra palabraCompleta, Collection<Palabra> palabras2) {
